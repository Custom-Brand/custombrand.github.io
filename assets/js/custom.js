(function($) {
    ...
    function preloader() {
        ...
        $wind.on("load", function() {
            ...
            gsap.timeline()
                ...
                .to(preloader.find('> *:not(.preloader-bar)'), {
                    y: -30,
                    autoAlpha: 0
                })
                .call(function() {
            ...
            nextProject: function() {
                ...
                if (_next.find('.bg-container').length)
                    tween.to(obj, {
                        value: 60,
                        onUpdate: function() {
                            _next.find('.bg-container').css('clip-path', 'circle(' + obj.value + '% at 30% ' + (obj.value - 20) + '%)');
                        }
                    }, 0);

                if (_nextProject.length) {
                    tween.fromTo(_nextProject, {
                        width: 0
                    }, {
                        width: "100%"
                    }, 0);
            ...
            dsnScrollTop: function() {
                ...
                $('.text-stroke-box > .text-stroke-inner').each(function() {
                    ...
                    if (!$(this).hasClass('dsn-animate'))
                        gsap.set(this, {
                            y: -1 * $(this).height() + $(this).parent().height()
                        });
                });

                tl.to(".scroll-to-top > img", 1, {
                    rotation: wrap.height() / 2,
                    ease: easeAnimate
                }, 0);
                tl.to(".text-stroke-box > .text-stroke-inner:not(.dsn-animate)", 1, {
                    y: 0
                }, 0);
    ...
    async function linkRightPaginate() {
        ...
        $('[data-dsn-title]').each(function() {
            ...
            _element.html($title);
            dsnGrid.convertTextLine(_element, 'words')
